...
app.get(
  "/api/autocomplete/:cityNameInput/limit/:size",
  cors(),
  async (req, res) => {
    ...
    if (!isNaN(cityName))
      return res
        .status(400)
        .json({ msg: "wrong search Input, input must not include numbers" });
...
app.get("/api/weatherForecast/lat/:lat/lon/:lon/", cors(), async (req, res) => {
  ...
  try {
    ...
    const prettifyNumericByAddingZero = (dayNumericForPrettify) => {
      if (dayNumericForPrettify < 10)
        return "0" + dayNumericForPrettify.toString();
    ...
    const forecast8days = results.data.daily.map((day, i) => {
      ...
      return {
        ...
        dayNumeric: prettifyNumericByAddingZero(
          dateOfTheDayFormatted.getDate()
        ),
        monthNumeric: prettifyNumericByAddingZero(
          dateOfTheDayFormatted.getMonth() + 1
        ), //January gives 0, so need to increase by 1 for correct month number
    ...
    const dateByTimeZone = new Date(currentDateFormatted).toLocaleString(
      "en-US",
      {
    ...
    const relevantData = {
      ...
      current: {
        ...
        monthNumeric: prettifyNumericByAddingZero(
          currentDateFormatted.getMonth() + 1
        ), //January gives 0, so need to increase by 1 for correct month number
    ...
    if (err.response.status === 429) {
      ...
      return res
        ...
        .json({
          data: {
            err: "429",
            msg: "Too Many GET Requests to Weather Maps API",
          },
        });
